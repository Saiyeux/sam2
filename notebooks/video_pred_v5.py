...
# 解析命令行参数
parser = argparse.ArgumentParser(description="使用SAM 2进行视频分割，分块处理以防止OOM，直接使用指定边界框作为提示")
...
# 验证第一帧的边界框
print("加载第一帧以验证边界框...")
first_frame_img = np.array(first_frame.convert("RGB"))
try:
...
# 分块处理视频
BLOCK_SIZE = 100  # 每块100帧
num_blocks = (len(frame_names) + BLOCK_SIZE - 1) // BLOCK_SIZE
print(f"将视频分为 {num_blocks} 块，每块最多 {BLOCK_SIZE} 帧")
...
for block_idx in range(num_blocks):
    ...
    print(f"\n处理块 {block_idx + 1}/{num_blocks} (帧 {start_frame} 至 {end_frame - 1})")
    
    # 为块帧创建临时目录
    temp_dir = tempfile.mkdtemp()
    try:
        print(f"将 {len(block_frames)} 帧复制到临时目录 {temp_dir}...")
        ...
        # 为块的首帧添加边界框提示
        obj_id = 1
        print(f"在块首帧为对象 {obj_id} 添加边界框提示")
        ...
        for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(inference_state):
            ...
            if out_frame_idx % 50 == 0:
                print(f"已处理块内帧 {global_frame_idx}/{end_frame - 1}")
                if device.type == "cuda":
                    used_mb, total_mb = get_gpu_memory()
                    print(f"帧 {global_frame_idx} 的GPU内存: {used_mb:.2f}/{total_mb:.2f} MB")
        print(f"块传播完成，耗时 {datetime.now() - start_time}")
        ...
        if device.type == "cuda":
            torch.cuda.empty_cache()
            used_mb, total_mb = get_gpu_memory()
            print(f"块清理后的GPU内存: {used_mb:.2f}/{total_mb:.2f} MB")
