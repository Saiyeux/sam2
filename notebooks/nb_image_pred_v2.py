...
def main():
    """主函数：处理命令行参数并执行批量图片分割，记录总推理时间，最后生成视频"""
    # 设置随机种子，确保结果可复现
    np.random.seed(3)
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="使用 SAM2 对图片进行自动分割并生成视频")
    ...
    segmented_images = [f for f in os.listdir(args.output_dir) if f.endswith('_segmented.png')]
    if not segmented_images:
        logger.error("错误: 未找到分割图像，无法生成视频")
        sys.exit(1)
    ...
    # 读取第一张图像以获取分辨率
    first_image_path = os.path.join(args.output_dir, segmented_images[0])
    ...
    for img_file in segmented_images:
        ...
        if frame is None:
            logger.warning(f"警告: 无法读取图像 {img_path}，跳过")
            continue
        if frame.shape[:2] != (height, width):
            logger.warning(f"警告: 图像 {img_path} 分辨率不一致，调整大小")
            frame = cv2.resize(frame, (width, height))
        video_writer.write(frame)
        logger.info(f"已添加帧: {img_path}")
